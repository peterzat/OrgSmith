...
from ..doctext import DocText, eml_message as _eml_message
...
class Context:
    ...
    # The shared extractable-text reader (orgsmith/doctext.py), built on
    # first use so a Context assembled for a rule that needs no text pays
    # nothing. Evals and baselines read the same module, so all three
    # consumers see one answer per document.
    _doctext: object = None
    ...
    def scan_archive(self, entry) -> list | None:
        ...
        return self.doctext.scan_archive(entry)
